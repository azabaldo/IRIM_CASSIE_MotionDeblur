function [inputs, scores] = run_One_Equal_3(t, xX, vX, xY, vY)
global runInfo
...
for i = 1:1:length(all_OCRText)
    for j = 1:1:size(all_OCRText{i},1)
        if j == 1
            select_Conf{i} = all_Conf{i}(j,:);
            select_OCRText{i} = all_OCRText{i}(j,:);
            select_Bound{i} = all_Bound{i}(:,:,j);
        else
            select_Conf{i} = [select_Conf{i}; all_Conf{i}(j,:)];
            select_OCRText{i} = [select_OCRText{i}; all_OCRText{i}(j,:)];
            select_Bound{i} = cat(3, select_Bound{i}, all_Bound{i}(:,:,j));
        end
    end
end

%find the combined string for each image
strings = combineStrings(select_OCRText, select_Conf, select_Bound, 3);
...
for i = 1:1:length(select_strings)
    ...
    matches = [];
    mapping = [];
    for j = 1:1:size(select_bound{i},3)-1
        for k = j+1:1:size(select_bound{i},3)
